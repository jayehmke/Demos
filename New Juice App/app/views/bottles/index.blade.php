	...
	<div class="col-md-12">
		<div class="panel panel-default">
		    ...
		    @foreach($bottles as $bottle)
		        <tr>
			        <td>{{ $bottle->name }}</td>
			        <td>{{ $bottle->case_size}}</td>
			        <td>{{ $bottle->current_level}}</td>
			        <td>{{ $bottle->par_level}}</td>
			        <td>{{ $bottle->alert_level}}</td>
			        <td>
				        <a class="btn btn-primary btn-lg" href="{{ URL::to('bottles/' . $bottle->id . '/edit') }}" role="button"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
				        <a class="btn btn-primary btn-lg" href="#" role="button"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
				       
						{{ link_to_route('bottles.destroy', 'D', $bottle->id, ['data-method'=>'delete']) }}
